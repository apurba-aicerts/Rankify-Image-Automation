...
Then open **http://localhost:8760** (set API base + `x-api-key` in the page). See [`frontend/README.md`](frontend/README.md).
...
| Service   | URL |
|-----------|-----|
| Frontend  | http://localhost:8760 (nginx proxies `/api` to the API) |
| API       | http://localhost:8750 |
| Swagger   | http://localhost:8750/docs |
...
Point the frontend at `http://localhost:8750` (or your deployed API URL) and send `x-api-key` on protected routes as documented in the backend README.
